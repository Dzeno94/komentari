...
function renderBlogs(blogs) {
    ...
    for (var blog of blogs) {
        ...
        var omotkom= document.createElement("div");
        var inputKom= document.createElement("input");
        inputKom.type="text";
        inputKom.placeholder="Comment this post";
        inputKom.style="width:100%;"
        
        for(var komentar of blog.komentari){
            var okvirKom= document.createElement("div");
            var textKom=document.createElement("p");
            var autorKom=document.createElement("span");
            var datumKom=document.createElement("i");
            datumKom.style.paddingLeft="30px";
            datumKom.innerHTML= new Date(komentar.datum).toLocaleString();
            okvirKom.classList.add("okvir-kom");
            textKom.classList.add("text-kom");
            autorKom.innerHTML=komentar.author;
            textKom.innerHTML=`Komentar: ${komentar.text}`;
            okvirKom.appendChild(textKom);
            okvirKom.appendChild(autorKom);
            okvirKom.appendChild(datumKom);
            omotkom.appendChild(okvirKom);

            

        }
        inputKom.addEventListener("keyup", function komEnter(e){
            if(e.key ==="Enter"){
                var komentar = {
                    author:loggedUser.name,
                    text:e.target.value,
                ...
                for (var cijeliBlog of allBlogs){
             if(this.parentNode.firstChild.innerHTML ==cijeliBlog.blogDesc){
                 cijeliBlog.komentari.push(komentar);
             }
             localStorage.setItem("blogs",JSON.stringify(allBlogs));
